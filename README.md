...
PRD 3.1.2 目前只有历史原型和实验基线，尚未形成符合正式技术栈的交付实现。已有 Fashionpedia 19 类局部 Mask、人物 ROI、Grounding DINO 候选框、SAM-HQ Mask 细化、Mask 派生 Box 和 `POST /v1/localize` 路由；这些资产可用于接口和失败分析，但 Grounding DINO、Fashionpedia 固定类别实验与 Hybrid 都不是 PRD 指定的 `DINOv2 区域特征 + 文本特征相似度匹配 + SAM-HQ` 正式路径。后续必须使用 PRD 指定的 DeepFashion2 数据边界和人工标注定位测试集，在 Python 3.10.12 中实现主路径，再进行 ONNX Runtime/TensorRT 8.6.1 优化；只有人工测试集准确率达到 `92%`、定位时间达到 `30 ms`，且系统满足单卡 RTX 3090 `60 QPS` 与平均响应 `400 ms`，才能声明达标。
