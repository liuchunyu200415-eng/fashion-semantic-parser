...
PRD 3.1.2 目前只有历史原型和实验基线，尚未形成符合正式技术栈的交付实现。已有 Fashionpedia 19 类局部 Mask、人物 ROI、Grounding DINO 候选框、SAM-HQ Mask 细化、Mask 派生 Box 和 `POST /v1/localize` 路由。Fashionpedia 与 DeepFashion2 都可以用于训练、开发验证和候选区域监督；但 Grounding DINO 与固定类别 Hybrid 仍不是 PRD 指定的 `DINOv2 区域特征 + 文本特征相似度匹配 + SAM-HQ` 正式路径。后续必须在 Python 3.10.12 中实现主路径，再进行 ONNX Runtime/TensorRT 8.6.1 优化；只有独立人工标注测试集准确率达到 `92%`、定位时间达到 `30 ms`，且系统满足单卡 RTX 3090 `60 QPS` 与平均响应 `400 ms`，才能声明达标。

导师反馈后，3.1.2 的任务边界已修正为开放语言指代表达定位，而不是用自然语言选择固定 N 类部位。现有 19 类和 Hybrid 路径只保留为历史辅助基线；`targeted_3000` 是最佳固定类别实验模型，但不属于最终 3.1.2 主路径。现已增加 Fashionpedia 语言定位训练索引生成器和官方源 Mask 加载器，按图片构建基础、方位、直接属性和可靠服装—部件关系表达，使用 annotation ID 回查并校验图像、类别、Box 和 Mask，避免重复存储大规模 Polygon/RLE。下一阶段在该数据契约上实现 PRD 指定的 `DINOv2 区域特征 + 完整文本特征匹配 + SAM-HQ`，并将功能 smoke 与 `92%`、`30 ms` 正式验收分开。
