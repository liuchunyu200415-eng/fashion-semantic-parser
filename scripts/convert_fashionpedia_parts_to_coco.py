"""Audit or convert Fashionpedia parts for PRD 3.1.2 localization."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Audit Fashionpedia local-part masks or convert them to the PRD "
            "3.1.2 COCO schema."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Project-relative output path. Defaults to the localization path.",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        "--audit-only",
        action="store_true",
        help="Report part coverage without requiring image files.",
    )
    return parser.parse_args()


def main() -> None:
    """Run the Fashionpedia local-part audit or conversion."""
    args = parse_args()
    ...
    if args.audit_only:
        ...
        output_path = args.output or (
            ...
            f"fashionpedia_parts_{args.split}.json"
        )
